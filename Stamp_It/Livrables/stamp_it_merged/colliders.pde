...
void contactStarted(FContact c) {
  ...
  if (!debounceContact){  // avoid to much calls
    debounceContact = true ;
    if (players){ // on multi mode, determine which plateform is collided, to give points to right player
    ...
    if(c.getSeparation() < 0.2){ // avoid to much calls on collides, with that there is still some calls but fewer
      ...
      trucs.add(new TrucPousse(c.getX(), c.getY(), c_size)); // add animation
