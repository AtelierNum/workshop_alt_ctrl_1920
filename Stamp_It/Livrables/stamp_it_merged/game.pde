...
void looseGame(){
  screenLoose = true ;
  timer = CountdownTimerService.getNewCountdownTimer(this).configure(100, 10000).start(); // initialize countdown timer to display screen loose
  startGame = !startGame ;
}
...
void displayScore() {
  ...
  if (players){  // on mutli each on corner
    textFont(fbold);
    textAlign(LEFT);
    fill(0);
    textSize(60);
...
screenLoose = true ;
  ...
  if (players){ 
    ...
    if (isPlayerGuilty){ // when a player push out from top screen, display only score of other player and scoff the looser
    ...
    } else { // else determine which is the winner by score
      xblue = height/2 - 300;
      xred = height/2 + 300;
      if (score1 < score2){
        fill(red);
  ...
  } else { // on solo mode, give a sentence retribution according to his level
    fill(0);
    textSize(90);
    textAlign(CENTER);
    textFont(f);
